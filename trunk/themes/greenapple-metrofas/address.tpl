                    ...
                    <fieldset class="account_creation">
                            ...
                            {if $field_name eq 'address2'}
                                <p class="required text">
                                    ...
                                    <input type="text" id="address2" name="address2"
                                           value="{if isset($smarty.post.address2)}{$smarty.post.address2}{else}{if isset($address->address2)}{$address->address2}{/if}{/if}"/>
                                </p>
                            {/if}
                        ...
                        <p class="text clearfix">
                            ...
                            <input type="text" id="phone" name="phone"
                                   value="{if isset($smarty.post.phone)}{$smarty.post.phone}{else}{if isset($address->phone)}{$address->phone}{/if}{/if}"/>
                        </p>

                        <p class="{if $one_phone_at_least}required {/if}text clearfix">
                            <label for="phone_mobile">{l s='Mobile phone'}{if $one_phone_at_least}<sup>*</sup>{/if}
                            </label>
                            <input type="text" id="phone_mobile" name="phone_mobile"
                                   value="{if isset($smarty.post.phone_mobile)}{$smarty.post.phone_mobile}{else}{if isset($address->phone_mobile)}{$address->phone_mobile}{/if}{/if}"/>
                        </p>

                        <p class="required text clearfix" id="adress_alias">
                            <label for="alias">{l s='Please assign an address title for future reference.'} <sup>*</sup></label>
                            <input type="text" id="alias" name="alias"
                                   value="{if isset($smarty.post.alias)}{$smarty.post.alias}{else if isset($address->alias)}{$address->alias}{else if !isset($select_address)}{l s='My address'}{/if}"/>
                        </p>

                        <p class="submit2">
                            {if isset($id_address)}<input type="hidden" name="id_address"
                                                          value="{$id_address|intval}" />{/if}
                            {if isset($back)}<input type="hidden" name="back" value="{$back}" />{/if}
                            {if isset($mod)}<input type="hidden" name="mod" value="{$mod}" />{/if}
                            {if isset($select_address)}<input type="hidden" name="select_address"
                                                              value="{$select_address|intval}" />{/if}
                            <button type="submit" name="submitAddress" id="submitAddress" class="button">
                                <span><span>{l s='Save'}</span></span></button>
                        </p>
                    </fieldset>
                </form>
            </div>
