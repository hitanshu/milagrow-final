...
*}
<style>
    a.transfrom_loyality_point:hover {
        color: #fff !important;
    }
</style>
...
<div id="pagination" class="pagination">
	{if $nbpagination < $orders|@count}
		...
		{else}
			<li id="pagination_next" class="disabled"><span style="float: left">{l s='Next' mod='loyalty'}&nbsp;&raquo;</span></li>
			{*<li id="pagination_next" class="disabled">{l s='Next' mod='loyalty'}&nbsp;&raquo;</li>*}
		{/if}
		</ul>
	{/if}
	{*{if $orders|@count > 10}*}
...
<br />{l s='Vouchers generated here are usable in the following categories : ' mod='loyalty'}<br>
{if $categories}{$categories}{else}{l s='All' mod='loyalty'}{/if}

{if $transformation_allowed}
<p style="text-align:center; margin-top:20px">
	<a class="exclusive_large transfrom_loyality_point" href="{$link->getModuleLink('loyalty', 'default', ['process' => 'transformpoints'])}" onclick="return confirm('{l s='Are you sure you want to transform your points into vouchers?' mod='loyalty' js=1}');">{l s='Transform my points into a voucher of' mod='loyalty'} <span class="price">{convertPrice price=$voucher}</span>.</a>
</p>
{/if}
