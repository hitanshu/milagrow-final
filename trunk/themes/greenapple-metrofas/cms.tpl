                                ...
                                {if isset($sub_category) & !empty($sub_category)}
                                    ...
                                    <ul class="bullet">
                                        {foreach from=$sub_category item=subcategory}
                                            <li class="cms-list">
                                                <span class="arrow-cms-list">&raquo;</span> <a href="{$link->getCMSCategoryLink($subcategory.id_cms_category, $subcategory.link_rewrite)|escape:'htmlall':'UTF-8'}">{$subcategory.name|escape:'htmlall':'UTF-8'}</a>
                                            </li>
                                        {/foreach}
                                    </ul>
                                {/if}
                                {if isset($cms_pages) & !empty($cms_pages)}
                                    ...
                                    <ul class="bullet">
                                        {foreach from=$cms_pages item=cmspages}
                                            <li class="cms-list">
                                                <span class="arrow-cms-list">&raquo;</span> <a href="{$link->getCMSLink($cmspages.id_cms, $cmspages.link_rewrite)|escape:'htmlall':'UTF-8'}">{$cmspages.meta_title|escape:'htmlall':'UTF-8'}</a>
                                            </li>
                                        {/foreach}
                                    </ul>
                                {/if}
