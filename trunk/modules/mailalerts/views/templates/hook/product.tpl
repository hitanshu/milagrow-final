...
{if (!$notifyProduct['allow_oosp'] && $notifyProduct['quantity'] <= 0) OR !$notifyProduct['available_for_order']}
    {if isset($notifyEmail) AND $notifyEmail}
        <input type="text" id="oos_customer_email" name="customer_email" size="20"
               placeholder="Please Enter Email Id" class="mailalerts_oos_email" onclick="clearText();"/>
    {/if}
    <button class="button notify_me_btn" type="button" name="Submit" id="mailalert_link" onclick="return addNotification();">
        <span><span>Notify me when available</span></span></button>
{*<a href="#" onclick="return addNotification();"*}
{*id="mailalert_link">{l s='Notify me when available' mod='mailalerts'}</a>*}
    <br>
