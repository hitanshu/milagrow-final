...
class MailAlerts extends Module
{
    ...
    public function hookActionValidateOrder($params)
    {
        ...
        $order = $params['order'];
        if($order->payment=='COD'||$order->payment=='cod')
            return;
    ...
    public function hookActionProductOutOfStock($params)
    {
        ...
        $product = Product::getProductProperties((int)$context->cookie->id_lang, array('id_product' => $id_product), $context);
        if (!(int)Context::getContext()->cookie->isLogged())
            $this->context->smarty->assign('notifyEmail', 1);
