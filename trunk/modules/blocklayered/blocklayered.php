        ...
        foreach ($shop_list as $id_shop) {
            ...
            foreach ($currency_list as $currency) {
                $price = Product::priceCalculation($id_shop, (int)$id_product, null, null, null, null,
                    ...
                    $specific_price_output, true, 0, true, 0, 1);
                $discount = $specific_price_output['reduction'];
                $discount = Tools::convertPrice($discount, $currency['id_currency']);
                $price += $discount;
                $price = $price + (($price * $max_tax_rate) / 100);
                $price-=$discount;
            ...
            foreach ($product_min_prices as $specific_price)
                foreach ($currency_list as $currency) {
                    ...
                    $price = Product::priceCalculation((($specific_price['id_shop'] == 0) ? null : (int)$specific_price['id_shop']), (int)$id_product,
                        ...
                        $specific_price['from_quantity'], false, 10, false, true, true, $specific_price_output, true, 0, true, 0, 1);
                    if (!isset($max_price[$currency['id_currency']]))
                        $max_price[$currency['id_currency']] = 0;
                    if (!isset($min_price[$currency['id_currency']]))
                        $min_price[$currency['id_currency']] = null;
            ...
            foreach ($groups as $group)
                foreach ($currency_list as $currency) {
                    $price = Product::priceCalculation(null, (int)$id_product, null, null, null, null, (int)$currency['id_currency'], (int)$group['id_group'],
                        null, false, 10, false, true, true, $specific_price_output, true);
...
//					' . (int)Tools::ps_round($max_price[$currency['id_currency']] * (100 + $max_tax_rate) / 100, 0) . ')';
                $values[] = '(' . (int)$id_product . ',
					...
					' . $id_shop . ',
					' . (int)$min_price[$currency['id_currency']] . ',
					' . (int)$max_price[$currency['id_currency']] . ')';
    ...
    public function getProductByFilters($selected_filters = array())
    {
        ...
        if ($this->nbr_products == 0)
			...
			SELECT
			    ...
			    i.id_image,
			    il.legend,
			    m.name manufacturer_name,AVG(pc.`grade`) as rating,Count(DISTINCT(pc.`id_product_comment`)) as reviews,
			   ...
			   LEFT JOIN ' . _DB_PREFIX_ . 'manufacturer m ON (m.id_manufacturer = p.id_manufacturer) LEFT JOIN `' . _DB_PREFIX_ . 'product_comment` pc ON (pc.`id_product`=p.`id_product`)
			   WHERE ' . $alias_where . '.`active` = 1 AND
			   ' . (Configuration::get('PS_LAYERED_FULL_TREE') ? 'c.nleft >= ' . (int)$parent->nleft . '
			   AND c.nright <= ' . (int)$parent->nright : 'c.id_category = ' . (int)$id_parent) . '
			   AND c.active = 1
			   AND pl.id_lang = ' . (int)$cookie->id_lang . '
			   AND (pc.`deleted` = 0 OR pc.`deleted`is NULL)
    ...
    public static function filterProductForOutOfStock($products)
    {
        $normalFilterProducts = array();
        $outOfStockProducts = array();
        foreach ($products as $product) {
            if ((!$product['allow_oosp'] && $product['quantity'] <= 0) OR !$product['available_for_order'])
                $outOfStockProducts[] = $product;
            else
                $normalFilterProducts[] = $product;
        }

        if (!empty($outOfStockProducts))
            return array_merge($normalFilterProducts, $outOfStockProducts);
        else
            return $normalFilterProducts;
    ...
    public function getProducts($selected_filters, &$products, &$nb_products, &$p, &$n, &$pages_nb, &$start, &$stop, &$range)
    {
        ...
        $products = $this->filterProductForOutOfStock($products);
